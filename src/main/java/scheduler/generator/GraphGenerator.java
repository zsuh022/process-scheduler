...
/**
 * The {@code GraphGenerator} class is responsible for generating different types of random graphs, including stencil
 * graphs, fork/join graphs, and chain graphs. It provides methods for creating graphs with random layers, nodes, and
 * edges, and for saving these graphs as DOT files.
 */
public class GraphGenerator {
    ...
    /**
     * Initializes the number of nodes and edges for the graph by generating random values within defined upper and
     * lower bounds.
     */
    ...
    /**
     * Generates a stencil graph with a random number of layers and nodes per layer, and then outputs the graph to a DOT
     * file located in the resources folder as specified by the InputOutputParser.
     *
     * @throws IOException if an I/O error occurs while writing the graph to a DOT file.
     */
    public static void createStencilGraph() throws IOException {
    ...
    /**
     * Displays information about the generated graph, including the number of nodes, edges, and processors.
     */
    ...
    /**
     * Generates a random number of source nodes, which is calculated as a percentage of the total number of nodes.
     *
    ...
    private static int getRandomNumberOfLayers() {
        double randomPercentage = Utility.getRandomPercentage(LAYER_RATIO_LOWER_BOUND, LAYER_RATIO_UPPER_BOUND);
        return Math.max(2, (int) (randomPercentage * GraphInformation.numberOfNodes));
    }

    /**
     * Calculates the maximum number of directed acyclic graph (DAG) edges, which is half of the maximum number of
     * non-DAG edges.
     *
    ...
    /**
     * Calculates the maximum number of non-DAG edges, which is determined by the total number of nodes.
     *
    ...
    /**
     * Generates a random weight value for nodes and edges, based on predefined upper and lower bounds.
     *
    ...
    /**
     * Generates random permutations of source nodes, ensuring a certain number of nodes are marked as source nodes.
     *
    ...
    /**
     * Creates an independent graph where nodes are not connected to each other, and writes it to a DOT file.
     *
     * @throws IOException if an I/O error occurs while writing the graph to a DOT file.
     */
    ...
    /**
     * Creates a chain graph where each node is connected to the next node, and writes it to a DOT file.
     *
     * @throws IOException if an I/O error occurs while writing the graph to a DOT file.
     */
    ...
    /**
     * Returns the filename suffix for the DOT file, which includes the number of nodes and edges in the graph.
     *
    ...
    /**
     * This class holds metadata about the generated graph, such as the number of nodes, edges, and processors.
     */
